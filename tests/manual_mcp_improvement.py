    ...
    try:
        ...
        try:
            result = await client.search_facilities(
                query="", facility_type="water_level", limit=10
            )
            print(f"✅ SUCCESS! Retrieved {len(result)} water_level facilities")
            if result:
                print(
                    f"   First result: {result[0].get('site', result[0]).get('site_name')}"
                )
        ...
        try:
            result = await client.search_facilities(
                query="춘천", facility_type="water_level", limit=10
            )
        ...
        try:
            stations = await client.find_related_stations(
                dam_name="소양강댐", direction="downstream", limit=5
            )
        ...
        try:
            stations = await client.find_related_stations(
                dam_name="팔당댐", direction="downstream", limit=3
            )
